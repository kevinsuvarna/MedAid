// Master switch for every Groq call in the app (explanations, doctor
// questions, vision OCR fallback). Must be true for any screen to use real
// extracted-report data instead of falling back to the "Could not load
// response" placeholder — flip to false only for local UI work where you
// want to avoid hitting the real API.
const GROQ_ENABLED = true;

async function postToGroqRaw(payload) {
  if (!GROQ_ENABLED) return { ok: false, status: 0, detail: 'Groq disabled' };
  try {
