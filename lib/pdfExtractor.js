...
import { callOpenAiVisionDetailed } from '@/lib/openaiClient';
...
// 1.5x measurably wasn't enough: tested against a real report, it repeatedly
// misread or dropped values on tightly-packed adjacent rows (MCH vs MCHC —
// same font size, one line apart, each with a small "Calculated" caption
// crowding the row — and Basophils in the differential block), returning
// null or the wrong neighboring row's value in different combinations across
// repeated calls. 2.5x resolved every one of those fields correctly and
// consistently across repeated runs on the same image.
const VISION_RENDER_SCALE = 2.5;
const VISION_IMAGE_QUALITY = 0.9;
...
// the API call itself fails (offline, quota exhausted, malformed response) —
// degraded-but-available beats a hard failure.
const VISION_SYSTEM_PROMPT = `You are extracting structured data from a Complete Blood Count (CBC) lab report image. Labs format this report differently — column order, inline High/Low flags, multi-line parameter names, extra "previous result" columns, watermarks — so match fields by their clinical meaning, not by exact text layout. You may be given more than one image — they are pages of the same report; combine what you find across all of them.

Rules:
- Also read the patient header block (usually top-left of the first page): name, age, sex. These matter as much as the lab values — the app uses sex to pick the correct reference range for several fields, so don't skip it even though it's not in a results table. "name" is the patient's personal name only — never a field label like "ABHA NO", "PID", "Client Patient ID", or a code/registration number. If the actual name is redacted, blank, or you're not confident which text is the name, use null rather than guessing from a nearby label.
...
- If a field is not present on the report, or you cannot read it confidently, use null for it. Never guess a value — this applies especially to sex: only set it if the word "Male" or "Female" (or M/F) is actually printed near a "Sex" label; do not infer it from a name or anything else.
- MCH and MCHC are adjacent rows with easily-confused labels — double-check each against its unit, not just its row position: MCH is measured in pg (typically ~27-32), MCHC in g/dL (typically ~32-36). Never assign one row's value to the other field.
- age must be a plain integer. sex must be exactly "Male", "Female", or null.`;

// Enforced server-side via OpenAI's strict json_schema response mode — the
// API guarantees the response validates against this exactly, so (unlike
// the old Groq path) there's no markdown-fence stripping or malformed-JSON
// handling needed; a response either matches this shape or the call itself
// fails.
const CBC_JSON_SCHEMA = {
  type: 'object',
  properties: {
    patient: {
      type: 'object',
    ...
    rbc: {
      type: 'object',
      properties: {
        haemoglobin: { type: ['number', 'null'] },
        rbc_count: { type: ['number', 'null'] },
        pcv: { type: ['number', 'null'] },
        mcv: { type: ['number', 'null'] },
        mch: { type: ['number', 'null'] },
        mchc: { type: ['number', 'null'] },
        rdw_cv: { type: ['number', 'null'] },
      },
      required: ['haemoglobin', 'rbc_count', 'pcv', 'mcv', 'mch', 'mchc', 'rdw_cv'],
      additionalProperties: false,
    },
    wbc: {
      type: 'object',
      properties: {
        total_wbc: { type: ['number', 'null'] },
        neutrophils: { type: ['number', 'null'] },
        lymphocytes: { type: ['number', 'null'] },
        monocytes: { type: ['number', 'null'] },
        eosinophils: { type: ['number', 'null'] },
        basophils: { type: ['number', 'null'] },
      },
      required: ['total_wbc', 'neutrophils', 'lymphocytes', 'monocytes', 'eosinophils', 'basophils'],
      additionalProperties: false,
    },
    platelets: {
      type: 'object',
      properties: {
        platelet_count: { type: ['number', 'null'] },
        mpv: { type: ['number', 'null'] },
      },
      required: ['platelet_count', 'mpv'],
      additionalProperties: false,
    },
  },
  required: ['patient', 'rbc', 'wbc', 'platelets'],
  additionalProperties: false,
};
...
async function extractViaOpenAiVision(pages) {
  const pageCount = Math.min(pages.length, MAX_OCR_PAGES);
  const imageDataUrls = [];
  for (let i = 0; i < pageCount; i += 1) {
    imageDataUrls.push(
      await renderPageToImageDataUrl(pages[i], { scale: VISION_RENDER_SCALE, type: 'image/jpeg', quality: VISION_IMAGE_QUALITY })
    );
  }
  if (imageDataUrls.length === 0) return null;

  // One call with every page, not one call per page: OpenAI's rate limits
  // aren't the razor-thin per-image budget Groq's free tier had, so there's
  // no need for the page-splitting + cross-page merge that path required.
  const result = await callOpenAiVisionDetailed(
    VISION_SYSTEM_PROMPT,
    'Extract the CBC values from this report as JSON.',
    imageDataUrls,
    CBC_JSON_SCHEMA
  );
  ...
  try {
    ...
    // Shouldn't happen under strict json_schema mode, but a provider-side
    // hiccup returning non-JSON shouldn't crash the whole extraction.
    return null;
  }
...
export async function extractReportData({ forceRefresh = false, arrayBuffer = null } = {}) {
  if (arrayBuffer) {
    ...
    try {
      visionResult = await extractViaOpenAiVision(pages);
    } catch (e) {
      visionResult = null;
    }
